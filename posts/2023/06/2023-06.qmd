...
The relationship between the distance a boat travels ($D$), the time the button is held ($w$), and the total race time ($T$) is given by the formula:
$$D = w \cdot (T - w)$$
...
To find the number of ways to beat the record, we calculate the range between the ceiling of $w_{upper}$ and the floor of $w_{lower}$, subtracting $1$ to exclude the end point:
