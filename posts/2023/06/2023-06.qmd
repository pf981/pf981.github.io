...
df <-
  tibble(wait_time = seq(from = 0, to = race_duration, by = 1)) |>
  mutate(
...
# I think just color the x tick of 30
# ggtext element_markdown
ggplot(df, aes(wait_time, distance, fill = I(fill))) +
  geom_col() +
  geom_hline(aes(yintercept = record), data = NULL, col = "#BE4844", linetype = "longdash", size = 0.8) +
  annotate(
    "text",
    x = 30,
    y = record + 2,
    label = glue::glue("Record: {record}mm"),
    hjust = "right",
    vjust = "bottom"
  ) +
  annotation_custom(
    grid::textGrob(glue::glue("Race Duration: {race_duration}ms"), gp = grid::gpar(fontsize = 12)),
    xmin = 25,
    xmax = 25,
    ymin = -12,
    ymax= -12
