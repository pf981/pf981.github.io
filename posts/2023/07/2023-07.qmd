...
We'll need a mechanism to rank hands. We'll define a `primary_order` which is the rank of the hand type, and a `secondary_order` which is the rank of each card.

First, we define a function, `primary_order`, which ranks the hand's type. This is accomplished by analysing the frequency of each card in the hand, allowing us to classify the hand based on predefined patterns.
...
Sorting the hands according to their combined <b class='fw-bold' style='color: #1577A0;'>rankings</b>, we can determine each hand's bid multiplier. Calculate the <b class='fw-bold' style='color: #91672C;'>total winnings</b> as the sum of bids multiplied by the <b class='fw-bold' style='color: #1577A0;'>rank</b>.
...
Create a new ordering function, `hand_bid_order_wild`. Compute the max `primary_order` when replacing <b class='fw-bold' style='color: #BE4844;'>J</b>s with every other card.
