...
Gear up for a grid-based challenge. In [Advent of Code day 3]({{< meta problem_url >}}), we are tasked with deciphering engine schematics to fix a gondola lift. This puzzle is trickier than the previous ones, but still manageable. It's our first grid question and involves parsing numbers, getting adjacencies, and handling duplicates.
...
1. Combine contiguous digits into numbers
2. Get the neighbouring numbers of each symbol
3. De-duplicate and sum those numbers

Assume that no number has more than one symbol neighbour.

In the example, the `#` symbol is touching `633` twice in the `N` and `NE` directions. We need to be mindful to not double count.
  

<!-- me-2 -->

##### 1. Combine contiguous digits into numbers
Construct a dictionary, `numbers` which maps every digit's coordinate to its representation
